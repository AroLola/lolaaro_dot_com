...
function walkAndClean(dir) {
  ...
  files.forEach((file) => {
    ...
    if (stat.isDirectory()) {
      ...
      if (godaddyPattern.test(content)) {
        console.log(`Cleaning GoDaddy icons in: ${fullPath}`);

        // Strip out all the old GoDaddy icons
...
walkAndClean('.');
console.log('Optimization complete! Local /sitepics/blob-6ecdd09.png icon path implemented.');
